...
# Modified to use MariaDB 8/3/2026, Alexander Towle

import sys
import getpass
import mariadb  # pyodbc not easy to configure on mac, pypyodbc not encoding/decoding
...
class MariaDB:
    ...
    def start(self):
        ...
        if self.ssh_tunnel:
            ...
            self.tunnel.start()
            try:
                self.conn = mariadb.connect(host='127.0.0.1',database=self.db,port=self.port,
            ...
            try:  # connection start
                self.conn = mariadb.connect(host=self.host,database=self.db,port=self.port,user=self.uid,password=self.pwd)
                self.conn.set_charset_collation(self.charset[0],self.charset[1])
            except RuntimeError:
                print('start():ER3.ODBC')
                self.errors += 'start():ER3.ODBC' + '\n'
